...
function Row({ children }) {
  return <div className="reg-row">{children}</div>
}
  ...
  return (
    <form className="auth-form" onSubmit={handleSubmit} noValidate>
      ...
      <p className="auth-switch">
        ...
        <button type="button" className="auth-switch-btn" onClick={onSwitch}>Create one</button>
      </p>
      <div className="auth-hint">
        <span>Admin:</span> admin@itsystem.com / admin123
      </div>
    </form>
  )
}
    ...
    <div className="success-overlay">
      ...
      <div className="success-card">
        ...
        <div className="success-details">
          ...
          {user.phone && (
            <div className="success-detail-row">
              <span className="sdl">Phone</span>
              <span className="sdv">{user.phone}</span>
            </div>
          )}
          {user.company && (
            <div className="success-detail-row">
              <span className="sdl">Company</span>
              <span className="sdv">{user.company}</span>
            </div>
          )}
          {user.jobTitle && (
            <div className="success-detail-row">
              <span className="sdl">Title</span>
              <span className="sdv">{user.jobTitle}</span>
            </div>
          )}
          {user.department && (
            <div className="success-detail-row">
              <span className="sdl">Department</span>
              <span className="sdv">{user.department}</span>
            </div>
          )}
          {user.position && (
            <div className="success-detail-row">
              <span className="sdl">Position</span>
              <span className="sdv">{user.position}</span>
            </div>
          )}
          {user.specialty && (
            <div className="success-detail-row">
              <span className="sdl">Specialty</span>
              <span className="sdv">{user.specialty}</span>
            </div>
          )}
          {user.experience && (
            <div className="success-detail-row">
              <span className="sdl">Experience</span>
              <span className="sdv">{user.experience}</span>
            </div>
          )}
          {user.availability && (
            <div className="success-detail-row">
              <span className="sdl">Availability</span>
              <span className="sdv">{user.availability}</span>
            </div>
          )}
          <div className="success-detail-row">
      ...
      {isPending ? (
        <p className="success-note">
          You will be notified once an admin reviews your application.<br />
      ...
      <button className="btn btn-primary success-signin-btn" onClick={onLogin}>
        Go to Sign In
      </button>
    </div>
  )
...
function RegisterForm({ onSwitch }) {
  ...
  /* show success screen inside the same card */
  if (createdUser) {
    return <SuccessScreen user={createdUser} onLogin={() => onSwitch()} />
  }
            ...
            <>
              ...
              <Row>
                ...
                <Field label="Job title" error={errors.jobTitle}>
                  <input type="text" placeholder="e.g. Office Manager"
                ...
                <div className="radio-group">
                  {['email', 'phone', 'both'].map(v => (
                    <label key={v} className="radio-label">
                      <input type="radio" name="contact" value={v}
                        checked={contactMethod === v}
              ...
              <Field label="Typical IT needs" optional>
                <textarea placeholder="Describe the type of IT problems you usually face…"
                  value={itNeeds} onChange={e => setItNeeds(e.target.value)}
                  style={{ minHeight: 80 }} />
              </Field>
            </>
          )}
            ...
            <>
              ...
              <Row>
                ...
                <Field label="Years of experience" error={errors.experience}>
                  <select className="reg-select" value={experience}
              ...
              <Field label="Availability" error={errors.availability}>
                <div className="radio-group">
                  {['Full-time', 'Part-time', 'Freelance', 'On-call'].map(v => (
                    <label key={v} className="radio-label">
                      <input type="radio" name="avail" value={v}
            ...
            <>
              ...
              <Row>
                <Field label="Company" error={errors.manCompany}>
                  <input type="text" placeholder="Acme Corp"
                    value={manCompany} onChange={e => setManCompany(e.target.value)} />
                </Field>
              ...
              <Row>
                ...
                <Field label="Team size" optional>
                  <select className="reg-select" value={teamSize}
              ...
              <Field label="Years of management experience" optional>
                <select className="reg-select" value={manExp}
                  onChange={e => setManExp(e.target.value)}>
                  <option value="">Select…</option>
                  <option>Less than 1 year</option>
                  <option>1 – 3 years</option>
                  <option>3 – 5 years</option>
  ...
  return (
    <div className="auth-page">
      <div className={`auth-card${tab === 'register' ? ' auth-card-wide' : ''}`}>
        <div className="auth-brand">IT Task Management</div>
